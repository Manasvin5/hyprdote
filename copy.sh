...
while true; do
    ...
    case $WALL in
        [Yy])
            ...
            if git clone "https://github.com/Manasvin5/walpies.git"; then
                ...
                if cp -R walpies/wallpapers/* ~/Pictures/wallpapers/ >> "$LOG" 2>&1; then
                    echo "${NOTE} Wallpapers copied successfully." 2>&1 | tee -a "$LOG"
                    rm -rf walpies 2>&1 # Remove cloned repository after copying wallpapers
                    break
                else
